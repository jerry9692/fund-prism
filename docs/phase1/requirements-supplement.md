...
Phase 1 主体代码已完成（99 测试通过、ruff 零告警、check-data 全过），覆盖了数据接入、分析算法、研究包和 5 个 Tool API。但对照 v0.4 第 16 节的 8 个 MVP 验收场景，有 3 个缺口需要补充才能形成完整 MVP 闭环。

本文档定义这 3 个补充模块的需求，范围严格控制在 MVP 验收所需的最小实现。
...
一期不做"保存筛选模板""基金池管理"——只返回筛选结果列表。

#### 3.1.4 验收

1. 可按类型/规模/成立年限筛选，返回基金列表
2. 可按收益/回撤排序
3. 返回结构包含 `{fund_code, short_name, company, category, scale, manager, metrics}`
4. 无匹配时返回空列表而非报错
...
4. 所有导出文件包含元数据和免责声明
5. 导出路径默认为 `./exports/`，支持 `--output` 自定义
...
补充模块完成后，v0.4 第 16 节全部 8 个 MVP 验收场景应可执行：
...
| 7. 筛选基金保存候选池 | `curl POST /api/v1/funds/screen -d '{...}'` |
...
- 不做基金池持久化管理（`screen_funds` 只返回结果，不保存）
- 不做综合评分（仍在二期）
