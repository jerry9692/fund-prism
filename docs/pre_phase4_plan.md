...
## 4. Phase 1-3 评估清账（P4.3，Phase 4 开工前完成 ✅ 2026-08-17）

> 来源：2026-08-17 对照需求书 v0.4 的 Phase 1-3 完成度评估（P1 ≈95% / P2 ≈85% / P3 ≈85%，均经代码与测试复跑验证）。筛选原则：**工作量小、无外部数据源依赖、对 Phase 4 开发或质量门禁有益**；工作量大的能力边界项登记为长期欠账（§4.6），不在开工前阻塞。

### P4.3-1 反选基金口径闭环（§6.2.10） ✅ 已完成

**落地说明**：`reverse_lookup.py`（算法版本 0.1.0→0.2.0）新增 `time_range` 三口径：`latest_report`（默认）/`recent_1y_avg`（近一年各披露期简单均值）/`specified_date`（不晚于指定日期的最近一期，缺 report_date 报错）；输出补全 `fund_name`/`fund_company`/`manager_names`（同名经理去重）/`sub_category`/`rank_in_category_1y`（同二级分类近一年收益排名，rank.py 口径）/`report_date`/`periods_used`/`exposure_change_vs_prev`（相邻期暴露差分）；API 请求新增 time_range/report_date 参数（枚举校验），落库 time_range 同步。模拟口径降级规则不变。

### P4.3-2 研究模板补全（§6.3.6 / §12.3） ✅ 已完成

**落地说明**：`research_template.py`（0.1.0→0.2.0）新增两模板与新 dispatcher：`holding_change_watch`（持仓变化：相邻两期披露持仓增减持对比，复用 `analyze_disclosed_holdings` + 指纹持仓特征步骤）与 `risk_scan`（风险扫描：nav_metrics 回撤/修复天数/波动/夏普/卡玛 + anomaly_scan 集中度/持有人结构规则子集）。内置模板 5→7，seed 幂等。

### P4.3-3 看板市场环境面板接入指数/因子（§6.3.8） ✅ 已完成

**落地说明**：`dashboard.py`（0.1.0→0.2.0）`market_overview` 新增 `index_performance`（沪深300/中证500/中证1000 近 1 月/3 月涨跌，stock_daily 收盘价推导）与 `factor_trends`（风格/债券因子 7 个近 20 交易日累计，factor_return）；前端 HomePage 市场概览区新增宽基指数/因子趋势两栏展示（跌负标红）。冒烟：三大指数近 3 月 −4.0%/−6.7%/−11.1%，因子趋势 7 个全输出。

### P4.3-4 检索关键词扩展（§6.3.1） ✅ 已完成

**落地说明**：`/funds/search` 除代码/简称/全称外，新增基金经理姓名（fund_manager）、公司（fund_company name/short_name）、重仓股（fund_disclosed_holdings 名称/代码）关键词检索，结果附 `match_source`（fund/manager/holding）标注。

### P4.3-5 v2 端点 API 层测试补齐 ✅ 已完成

**落地说明**：新增 `tests/test_api/test_v2_phase3.py`（13 例）覆盖 fingerprint/similar/anomaly/pool/reverse_lookup/templates/dashboard 七组端点，每组 happy path + 边界/降级用例（未知基金 needs_review、不存在基金池 needs_review、无匹配 needs_review、非法 time_range 422、未知模板不 500）；另新增 `tests/test_api/test_search.py`（5 例）。

**验收汇总**（全量回归 617 passed，ruff 全绿）：真实库三口径反选字段齐备；看板指数/因子环境有数；risk_scan/holding_change_watch 模板执行 completed；新 seed 模板 7 个。

### 4.6 不建议开工前清账（长期欠账登记）

| 项 | 原因 | 归属 |
| --- | --- | --- |
| 持有人结构数据源（P1 gap） | 免费 A 级源不可得，需官方披露解析器，不可硬造（数据源诚实） | 长期 |
| 模拟持仓转债池（bond_weight_pct 恒 0，P2 gap） | 工作量大、与 Phase 4 债基因子弱耦合 | Phase 4/5 可选 |
| 动态归因真日度滚动（P2 gap） | 工作量大，模块自述 future work | Phase 5 |
| 交易能力经理历史/同类对比（P2 gap） | §6.2.4 扩展项，三假设核心已满足 | Phase 4/5 可选 |
| 失败案例库实体化（P2 gap） | 低优先，experiment_result 已有记录 | Phase 5 |
| Tool API dry-run/explain + JSONL 导出（P1 gap） | 与 Phase 5 Agent 化配套更合适 | Phase 5 |
| 评分短窗口产品化门禁（P2 gap） | 随每日增量数据自然积累，非工程欠账 | 自然演进 |
| 组合分析 §6.3.9（P3 gap） | 属 Phase 4 正题，非欠账 | Phase 4 |
...
| **B5** | P4.2-1 模板扩展 + P4.2-2 注册表 + P4.2-3 口径版本化 | B1（样本确认新 类型） | B4              |
| **B6** | P4.3-1~5 评估清账                                   | B2/B3（看板接指数因子） | 其余全部        |

### 5.3 完成标准（进入 Phase 4 前）
...
- [x] 反选基金三种时间口径 + 输出字段齐备（P4.3-1；time_range 三口径 + 名称/公司/经理/同类排名/历史变化）
- [x] 持仓变化/风险扫描两个研究模板可执行（P4.3-2；内置模板 5→7，真实库执行 completed）
- [x] 看板市场环境面板展示指数 + 因子数据（P4.3-3；三大指数涨跌 + 7 因子趋势，前端 HomePage 展示）
- [x] 检索支持经理/公司/重仓股关键词（P4.3-4；附 match_source 标注）
- [x] Phase 3 v2 端点 API 层测试补齐（P4.3-5；13 例覆盖七组端点降级路径）
