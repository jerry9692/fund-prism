...
二期不是一次性交付全部能力，而是分轮推进：先建立算法实验与数据模型闭环，再把通过验证的估计结果暴露为 v2 API，最后建设前端产品化视图。任何未完成回测或未满足阈值的估计模块，只能停留在实验结果和 `estimated_*` 字段中，不得进入默认高置信度结论、默认评分或排名。
...
| Phase 1 质量门禁 | ✅ Phase 1 完成时通过；进入 Phase 2 后每轮需重新验证 |
...
为避免范围膨胀，二期拆分为三个可验收小闭环：

| 轮次 | 目标 | 可交付能力 | 明确不交付 |
|------|------|------------|------------|
| P2A | 实验底座 | P2 ORM + 迁移、实验 CRUD、结果记录、失败样本库、前端一期 API 页面 scaffold | 不输出模拟持仓/评分正式结论 |
| P2B | 算法验证 | 模拟持仓披露期回测、动态归因实验、评分回测框架、v2 实验 API | 不进入默认 Research Packet 或默认排名 |
| P2C | 产品化视图 | 仅展示通过阈值的估计结果、综合评分解释页、回测页、手动校验页 | 不展示未达标估计结果为可用结论 |

P2A 的最低验收是“底座可运行、可构建、可测试”，不是“算法已经可信”。P2B 的最低验收是“知道哪些基金/算法不可用”，不是追求所有基金都能给出看似精确的结果。

## 4. 前端需求

### 4.1 总体要求

二期必须提供可通过浏览器访问的 Web 界面。P2A 先覆盖 Phase 1 已有 API 的核心页面；P2B/P2C 再逐步覆盖二期新增 API。前端页面不得绕过统一 API client 访问后端。
...
### M1 / P2A: 底座与前端一期页面（1-2 周）

1. P2 数据库模型与 Alembic 迁移一致，ORM 可直接读写新增表。
2. 实验管理 CRUD 可用，支持创建实验、记录结果、重跑、删除和失败样本摘要。
3. 前端可构建，可访问 Phase 1 API 对应核心页面。
4. 统一 API client 生效，切换后端 URL 只需改一处。

退出标准：`ruff check src tests`、`pytest`、`npm run build`、`fund-research check-data` 全部通过；实验管理至少有单元测试覆盖。

### M2 / P2B: 算法验证闭环（3-4 周）

1. 模拟持仓算法 + 披露期回测。
2. 动态归因实验（基于达标模拟持仓结果）。
3. 综合评分算法（分项指标 + 权重 + 标准化）和评分回测框架。

退出标准：30 只样本基金完成回测报告；失败样本、不可用原因、输入覆盖率、残差和阈值判断可审计。

### M3 / P2C: 受控产品化视图（2-3 周）

1. 二期新增 API 与页面：模拟持仓、动态归因、评分、评分回测、手动校验。
2. 页面只展示通过门禁的估计结果；未达标结果展示为 `needs_review` 或不适用。
3. 研究员手动校验记录作为 Evidence 或 reviewer annotation 保存。

退出标准：全部二期页面可访问，无白屏；估计结果标注、残差和 warnings 在 API 与前端一致展示。

### M4: 集成与验收（1-2 周）

1. E2E 测试。
2. 文档更新。
3. Phase 1 + Phase 2 全部门禁回归。

退出标准：同 Phase 1 的质量门禁全部通过，并额外通过前端构建和 P2 API 测试。
...
1. P2 ORM 模型继承统一 `Base`，迁移、ORM、实验管理代码引用同一组表定义。
2. 实验管理模块可导入、可创建/列出/记录/删除实验。
3. 前端依赖安装后 `npm run build` 通过。
4. Phase 1 API 页面 scaffold 可访问，且所有后端访问走统一 API client。
5. `ruff check src tests`、`pytest`、`fund-research check-data` 通过。
