...
def test_duckdb_experiment_result_accepts_bigint_experiment_id(tmp_path: Path) -> None:
    """ExperimentResult FK must accept the 63-bit IDs generated by the app."""
    db_path = tmp_path / "fund_research.duckdb"
    init_db(str(db_path))
    engine = create_engine_from_path(str(db_path))
    large_id = 3_000_000_000

    with engine.begin() as conn:
        conn.execute(
            AlgorithmExperiment.__table__.insert().values(
                id=large_id,
                experiment_name="bigint-id-test",
        ...
        conn.execute(
            ExperimentResult.__table__.insert().values(
                id=large_id + 1,
                experiment_id=large_id,
                fund_code="000001",
                calc_date=date(2026, 6, 14),
                is_success=False,
                metrics={},
        ...
        row = conn.execute(
            select(ExperimentResult.experiment_id).where(ExperimentResult.id == large_id + 1)
        ).one()

    assert row.experiment_id == large_id
